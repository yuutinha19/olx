...
bot.command('buscar', async (ctx) => {
    const userId = ctx.from.id;
    const url = ctx.message.text.split(' ')[1]; // Obtém a URL do produto enviada pelo usuário

    if (!url) {
        return ctx.reply('❌ Por favor, envie o comando assim: /buscar <URL do produto>');
    }

    ctx.reply('🔍 Buscando informações, aguarde...');
    ...
    try {
        ...
        const data = await page.evaluate(() => {
            return {
                vendedor: document.querySelector('.nome-do-vendedor')?.innerText || 'Não encontrado',
                nome: document.querySelector('.nome-do-produto')?.innerText || 'Sem nome',
                descricao: document.querySelector('.descricao')?.innerText || 'Sem descrição',
                valor: document.querySelector('.preco')?.innerText || 'Preço indisponível',
                imagem1: document.querySelector('.imagem1')?.src || '',
                imagem2: document.querySelector('.imagem2')?.src || ''
            };
        });

        await browser.close();

        // Criando um ID para o produto
        const produtoId = Math.random().toString(36).slice(2, 11);
        const produtosData = loadData();

        // Salvando os dados coletados, mas deixando a data e o QR em aberto
        produtosData.produtos[produtoId] = {
            id: produtoId,
            userId,
            etapa: 'data',
            ...data,
        ...
        saveData(produtosData);
        produtos[userId] = produtosData.produtos[produtoId];

        ctx.reply(
            `✅ Produto cadastrado automaticamente!\n\n📌 Nome: *${data.nome}*\n💰 Preço: *${data.valor}*\n🛒 Vendedor: *${data.vendedor}*\n\nAgora, digite a *data da venda* (ex: 28/01/2025):`,
            { parse_mode: 'Markdown' }
        );

    } catch (error) {
        console.error('Erro ao buscar dados:', error);
        ctx.reply('❌ Erro ao buscar os dados. O site pode não ser compatível.');
    }
});

// Captura a data da venda e depois pede o QR Code
    ...
    if (produtos[userId]) {
        ...
        switch (produto.etapa) {
            case 'data':
                ...
                saveData(data);
                await ctx.reply('📅 Data registrada! Agora, digite o *código QR* do pagamento:');
                break;
            ...
            case 'qr':
                ...
                saveData(data);
                await ctx.reply(
                    `✅ Produto finalizado!\n\n📌 Nome: *${produto.nome}*\n💰 Preço: *${produto.valor}*\n🛒 Vendedor: *${produto.vendedor}*\n📅 Data da venda: *${produto.dataVenda}*\n\n🔗 [Acesse o produto aqui](${RENDER_URL}/produtos?id=${produto.id})`,
                    { parse_mode: 'Markdown' }
                );
                break;

            default:
                await ctx.reply('⚠️ Comando não reconhecido.');
        }
    }
});
